...
- GroundOverlay: SDK가 발화하지 않는 dblclick/mouseover/mouseout/mousemove 이벤트 prop 제거 (타입 축소).
...
### 회귀 테스트
- 121개 → **147개** 통과. 모든 fix 시나리오 커버.

### Breaking 가능성 (사실상 없음)
- GroundOverlay 이벤트 타입 축소: 기존 코드에서 `onMouseover` 등을 사용했더라도 SDK가 발화하지 않았으므로 실제 동작 변화 없음.
